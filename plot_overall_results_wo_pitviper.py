...
overall_results_df = pd.read_csv(
    "results_wo_pitviper/Gesture_overall.csv", index_col=False
)
...
for model in models:
    ...
    plt.savefig(
        f"plots_wo_pitviper/Gesture_{model}_overall.png",
        bbox_inches="tight",
        dpi=600,
    )
    plt.savefig(
        f"plots_wo_pitviper/Gesture_{model}_overall.tiff",
        bbox_inches="tight",
        dpi=600,
    )
    plt.close()
