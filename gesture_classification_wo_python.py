...
# concatenate data
df = pd.concat([CV, PY], ignore_index=True)
...
for model, cv_n_split, test_size in product(models, cv_n_splits, test_sizes):
    ...
    for train, test in cv.split(X, classes, groups):
        ...
        if model == "SVC":
            ...
            auc = roc_auc_score(
                classes[test],
                scale_cf.predict_proba(X[test])[:, 1],
                multi_class="ovr",
            )
    ...
    results_df.to_csv(
        f"results_wo_python/Gesture_{name_str}_results_full.csv",
        index=False,
    )
    ...
    av_results_df.to_csv(
        f"results_wo_python/Gesture_{name_str}_results_average.csv"
    )
    ...
    # write to txt file
    with open(f"results_wo_python/Gesture_{name_str}_report.txt", "w") as f:
        f.write(report)
    ...
    # write to txt file
    with open(
        f"results_wo_python/Gesture_{name_str}_dummyreport.txt", "w"
    ) as f:
    ...
    plt.savefig(
        f"plots_wo_python/Gesture_{name_str}_accuracy.png",
        bbox_inches="tight",
        dpi=600,
    )
    plt.savefig(
        f"plots_wo_python/Gesture_{name_str}_accuracy.tiff",
        bbox_inches="tight",
        dpi=600,
    )
    plt.close()
...
overall_results_df.to_csv("results_wo_python/Gesture_overall.csv", index=False)
