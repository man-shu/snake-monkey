...
# concatenate data
df = pd.concat([BB, PY], ignore_index=True)
...
for model, cv_n_split, test_size in product(models, cv_n_splits, test_sizes):
    ...
    results_df.to_csv(
        f"results_wo_pitviper/Gesture_{name_str}_results_full.csv",
        index=False,
    )
    ...
    av_results_df.to_csv(
        f"results_wo_pitviper/Gesture_{name_str}_results_average.csv"
    )
    ...
    # write to txt file
    with open(f"results_wo_pitviper/Gesture_{name_str}_report.txt", "w") as f:
        f.write(report)
    ...
    # write to txt file
    with open(
        f"results_wo_pitviper/Gesture_{name_str}_dummyreport.txt", "w"
    ) as f:
    ...
    plt.savefig(
        f"plots_wo_pitviper/Gesture_{name_str}_accuracy.png",
        bbox_inches="tight",
        dpi=600,
    )
    plt.savefig(
        f"plots_wo_pitviper/Gesture_{name_str}_accuracy.tiff",
        bbox_inches="tight",
        dpi=600,
    )
    plt.close()
...
overall_results_df.to_csv(
    "results_wo_pitviper/Gesture_overall.csv", index=False
)
